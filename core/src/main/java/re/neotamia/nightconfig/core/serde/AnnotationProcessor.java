...
import static re.neotamia.nightconfig.core.serde.annotations.SerdePhase.*;
...
final class AnnotationProcessor {
    ...
    /**
     * Combines all {@link SerdeAssert} annotations that apply to the {@code currentPhase} into a {@link Predicate}.
     *
     * @param annotations     array of {@link SerdeAssert} annotations
     * @param currentInstance the object that is being (de)serialized
     * @param currentPhase    the phase to look for
     * @return a Predicate that combines every applicable assertion, or {@code null} if none applies
     */
    ...
    static Predicate<?> resolveAssertPredicate(SerdeAssert[] annotations, Object currentInstance, SerdePhase currentPhase, Field field) {
        ...
        for (SerdeAssert annot : annotations) {
            ...
            if (annotPhase == currentPhase || annotPhase == SerdePhase.BOTH) {
                for (AssertThat condition : conditions) {
                    predicates.add((Predicate<Object>) resolveAssertPredicate(condition, annot, currentInstance,
        ...
        void check(SerdeAssert annotation) {
            if (hasCustomParam && !hasCustomAssert) {
                throw new SerdeException(String.format(
                        "Invalid annotation %s: without AssertThat.CUSTOM, no additional parameter must be specified.", annotToString(annotation)));
            }
        }
    }

    private static Predicate<?> resolveAssertPredicate(AssertThat assertThat, SerdeAssert annotation, Object currentInstance, SerdePhase currentPhase,
                                                       Class<?> fieldType, SerdeAssertSanityCheck sanityCheck) {
    ...
    static Predicate<?> resolveSkipDeserializingIfPredicate(SerdeSkipDeserializingIf annotation, Object currentInstance) {
        ...
        for (int i = 0; i < predicates.length; i++) {
            ...
            predicates[i] = resolveSkipDeserializingIfPredicate(condition, annotation, currentInstance, Object.class);
        }
        return combineOr(predicates);
    }

    private static Predicate<?> resolveSkipDeserializingIfPredicate(SkipDeIf skipIf, SerdeSkipDeserializingIf annotation, Object currentInstance,
        ...
        if (skipIf == SkipDeIf.CUSTOM) {
            ...
            if (skipIf == SkipDeIf.ALWAYS) {
                return v -> true;
            } else if (skipIf == SkipDeIf.IS_MISSING) {
    ...
    // ====== SerdeSkip ======
    @SuppressWarnings({"rawtypes", "unchecked"})
    static Predicate<?> resolveSkipPredicate(SerdeSkip annotation, Object currentInstance, SerdePhase phase, Field field) {
        SerdeSkip.SkipIf[] conditions = annotation.value();
        Predicate[] predicates = new Predicate[conditions.length];
        for (int i = 0; i < predicates.length; i++) {
            SerdeSkip.SkipIf condition = conditions[i];
            predicates[i] = resolveSkipPredicate(condition, annotation, currentInstance, phase, field);
        }
        return combineOr(predicates);
    }

    private static Predicate<?> resolveSkipPredicate(SerdeSkip.SkipIf skipIf, SerdeSkip annotation, Object currentInstance, SerdePhase phase, Field field) {
        Class<?> cls = annotation.customClass();
        String methodOrFieldName = annotation.customCheck();

        if (skipIf == SerdeSkip.SkipIf.CUSTOM) {
            if (methodOrFieldName.isEmpty()) {
                throw new SerdeException(String.format(
                        "Invalid annotation %s: with SkipIf.CUSTOM, parameter `customCheck` must be provided and non-empty.", annotToString(annotation)));
            }
            Class<?> predicateParameterType = (phase == SerdePhase.SERIALIZING) ? field.getType() : Object.class;
            return findCustomPredicate("skip predicate", annotation, cls, methodOrFieldName, currentInstance, predicateParameterType);
        } else {
            if (!methodOrFieldName.isEmpty() || cls != Object.class) {
                throw new SerdeException(String.format(
                        "Invalid annotation %s: with SkipIf.%s, no additional parameter must be specified.", annotToString(annotation), skipIf.name()));
            }
            if (skipIf == SerdeSkip.SkipIf.ALWAYS) {
                return v -> true;
            } else if (skipIf == SerdeSkip.SkipIf.IS_NULL) {
                if (phase == SerdePhase.SERIALIZING) {
                    return Objects::isNull;
                } else {
                    return v -> v == NullObject.NULL_OBJECT;
                }
            } else if (skipIf == SerdeSkip.SkipIf.IS_EMPTY) {
                return v -> v != null && Util.isEmpty(v);
            } else {
                assert false : "missing case";
            }
    ...
    static Predicate<?> resolveSkipSerializingIfPredicate(SerdeSkipSerializingIf annotation, Object currentInstance, Field field) {
        ...
        for (int i = 0; i < predicates.length; i++) {
            ...
            predicates[i] = resolveSkipSerializingIfPredicate(condition, annotation, currentInstance, field.getType());
        }
        return combineOr(predicates);
    }

    private static Predicate<?> resolveSkipSerializingIfPredicate(SkipSerIf skipIf, SerdeSkipSerializingIf annotation, Object currentInstance,
                                                                  Class<?> fieldType) {
        Class<?> cls = annotation.customClass();
        String methodOrFieldName = annotation.customCheck();
        ...
        if (skipIf == SkipSerIf.CUSTOM) {
            ...
            if (skipIf == SkipSerIf.ALWAYS) {
                return v -> true;
            } else if (skipIf == SkipSerIf.IS_NULL) {
                return Objects::isNull;
    ...
    private static Predicate<?> predicateFromField(String label, Field field, Object instance, boolean mustBeStatic) {
    ...
    private static Predicate<?> findCustomPredicate(String label, Annotation annotation, Class<?> cls, String methodOrFieldName,
        ...
        return (methodOrField instanceof Field)
                ? predicateFromField(label, (Field) methodOrField, currentInstance, cls != Object.class)
    ...
    /**
     * Extracts SerdeSkip annotations from SerdeConfig and processes them.
     */
    static Predicate<?> resolveSerdeConfigSkipPredicate(SerdeConfig config, Object currentInstance, SerdePhase phase, Field field) {
        SerdeSkip[] skipAnnotations = config.skip();
        if (skipAnnotations.length == 0) return null;
        if (skipAnnotations.length > 1) throw new SerdeException("SerdeConfig can only contain one SerdeSkip annotation");
        return resolveSkipPredicate(skipAnnotations[0], currentInstance, phase, field);
    }

    /**
     * Extracts SerdeSkipDeserializingIf annotations from SerdeConfig and processes them.
     */
    static Predicate<?> resolveSerdeConfigSkipDeserializingIfPredicate(SerdeConfig config, Object currentInstance) {
        SerdeSkipDeserializingIf[] skipAnnotations = config.skipDeserializingIf();
        if (skipAnnotations.length == 0) return null;
        if (skipAnnotations.length > 1) throw new SerdeException("SerdeConfig can only contain one SerdeSkipDeserializingIf annotation");
        return resolveSkipDeserializingIfPredicate(skipAnnotations[0], currentInstance);
    }

    /**
     * Extracts SerdeSkipSerializingIf annotations from SerdeConfig and processes them.
     */
    static Predicate<?> resolveSerdeConfigSkipSerializingIfPredicate(SerdeConfig config, Object currentInstance, Field field) {
        SerdeSkipSerializingIf[] skipAnnotations = config.skipSerializingIf();
        if (skipAnnotations.length == 0) return null;
    ...
    static String annotToString(Annotation annotation) {
        return annotation.toString().replace("@re.neotamia.nightconfig.core.serde.annotations.", "@");
    }
}
